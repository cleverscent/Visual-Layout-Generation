...
def custom_collate_fn(batch, attention_mask):
    ...
    if attention_mask:
        ...
        batch_collated["cluster_image_id"] = batch_collated["cluster_image_id"][:, :max_non_zero]
        batch_collated["cluster_text_id"] = batch_collated["cluster_text_id"][:, :max_non_zero]
...
def masked_l2_rz(noise, noise_pred, mask, mask_img, image_noise, is_image_pred, device):
    # 첫 4개 요소에 대한 MSE 손실 계산
    mse_loss_bbox = F.mse_loss(noise[:, :, :4], noise_pred[:, :, :4], reduction='none') * mask[:, :, :4].float()
    mse_loss_bbox = sum_flat(mse_loss_bbox) / (sum_flat(mask[:, :, :4]) + 1e-8)
    
    mse_loss_r = F.mse_loss(noise[:, :, 4].unsqueeze(-1), noise_pred[:, :, 4].unsqueeze(-1), reduction='none') * mask[:, :, 4].unsqueeze(-1).float()
    mse_loss_r = sum_flat(mse_loss_r) / (sum_flat(mask[:, :, 4].unsqueeze(-1)) + 1e-8)

    mse_loss_z = F.mse_loss(noise[:, :, 5].unsqueeze(-1), noise_pred[:, :, 5].unsqueeze(-1), reduction='none') * mask[:, :, 5].unsqueeze(-1).float()
    ...
    if is_image_pred:
        mse_loss_img_features = F.mse_loss(image_noise, noise_pred[:, :, 6:], reduction='none') * mask_img.float()
