...
def main(*args, **kwargs):
    ...
    if config.dataset == 'canva':
        train_data = CanvaLayout(config.train_json, config.train_clip_json, config.train_text_json, 
                                 config.cluster_image_model,  config.cluster_text_model,
                                 max_num_com=config.max_num_comp, scaling_size=config.scaling_size, 
                                 z_scaling_size = config.z_scaling_size, mean_0 = config.mean_0)
        val_data = CanvaLayout(config.val_json, config.val_clip_json, config.val_text_json, 
                               config.cluster_image_model,  config.cluster_text_model,
                               max_num_com=config.max_num_comp, scaling_size=config.scaling_size, 
                               z_scaling_size = config.z_scaling_size, mean_0 = config.mean_0)
    else:
    ...
    TrainLoopCAL(accelerator=accelerator, model=model, diffusion=noise_scheduler, epsilon_scheduler=Epsilon_Scheduler,
             ...
             is_cond=config.is_cond, t_sampling=config.t_sampling, image_pred_ox=config.image_pred_ox, attention_mask=config.attention_mask).train()
...
def init_job():
    ...
    parser.add_argument('--cluster_image_model', type=str, default="clustering_with_image_model.pkl")
    parser.add_argument('--cluster_text_model', type=str, default="clustering_with_text_model.pkl")
    ...
    parser.add_argument('--work_dir', type=str, default="train")
    ...
    config.cluster_image_model = Path(config.dataset_path)/config.cluster_image_model
    config.cluster_text_model = Path(config.dataset_path)/config.cluster_text_model
